...
public class DaySeventeen {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        File file = new File("java/src/day17/DaySeventeen.txt");
    ...
    public static HashMap<String, Tesseract> cloneUniverse(HashMap<String, Tesseract> pocketUniverse) {
        HashMap<String, Tesseract> newDimension = new HashMap<>();
        for (Map.Entry<String, Tesseract> entry : pocketUniverse.entrySet())
            newDimension.put(entry.getKey(), entry.getValue().clone());
        return newDimension;
    }

    public static void loadNeighborTesseracts(HashMap<String, Tesseract> pocketUniverse, boolean hasUniverse) {
        List<Tesseract> ogTesseracts = new ArrayList<>(pocketUniverse.values());
        for (int w = -1; w <= 1; w++) {
                ...
                for (int y = -1; y <= 1; y++) {
                    for (int z = -1; z <= 1; z++) {
                        ...
                        for (Tesseract tesseract : ogTesseracts) {
                            ...
                            if (pocketUniverse.containsKey(location)) continue;
                            pocketUniverse.put(location, new Tesseract(tesseract.x() + x, tesseract.y() + y, tesseract.z() + z, tesseract.w() + w, false));
                        }
                    }
                }
            }
    ...
    public static List<Tesseract> getNeighborTesseracts(Tesseract tesseract, HashMap<String, Tesseract> pocketUniverse) {
            ...
            for (int x = -1; x <= 1; x++) {
                for (int y = -1; y <= 1; y++) {
                    for (int z = -1; z <= 1; z++) {
                        ...
                        String location = (tesseract.x() + x) + "," + (tesseract.y() + y) + "," + (tesseract.z() + z) + "," + (tesseract.w() + w);
                        if (!pocketUniverse.containsKey(location)) continue;
                        neighbors.add(pocketUniverse.get(location));
                    }
                }
            }
        }
